...
public class GallAnsBubTtableOut {    
	
	// MAIN 생성용 OBJECT
	private JSONObject jObjRtn = new JSONObject();
	
	public GallAnsBubTtableOut (JSONObject jobj) throws IOException {
	//public LonginTtableOut (String userId,String userType,String userPasswrd) throws IOException {
		...
		try {
			...
			{
				// 건의 게시판 답변 생성일련번호 채번
				long ansSeq = session.selectOne("uni-gallery-mapping.selectGallAnsBubSeq");
				param.put("ANS_SEQ", ansSeq);
				
				// 건의게시판 답변 등록
				session.insert("uni-gallery-mapping.insertGallAnsBubInfo",param);
			...
			if ("02".equals(jobj.getString("PROC_TYPE")))
			{
				...
				session.update("uni-gallery-mapping.updateGallAnsBubInfo",param);
			}
			...
			if ("03".equals(jobj.getString("PROC_TYPE")))
			{
				// 건의게시판 답변 삭제
				param.put("ANS_SEQ", jobj.get("ANS_SEQ"));
				session.delete("uni-gallery-mapping.deleteGallAnsInfo",param);
			}
