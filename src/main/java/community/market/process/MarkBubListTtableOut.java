...
public class MarkBubListTtableOut {    
	
	// MAIN 생성용 OBJECT
	private JSONObject jObjMain = new JSONObject();
	
	public MarkBubListTtableOut (Map<String, Object> param) throws IOException {
	//public LonginTtableOut (String userId,String userType,String userPasswrd) throws IOException {
		...
		try {
			...
			rtnList = session.selectList("uni-community-mapping.selectMarkBubInfo",param);	
			...
			for (int i=0; i < rtnList.size() ;i++)
			{
				...
				rtnListSub2 = session.selectList("uni-community-mapping.selectMarkImgInfo",param);	
				...
				// 공지사항이 loop 수행마다 해당 공지에 첨부된 이미지파일이 있을수 있으니 조회한다.
				rtnListSub3 = session.selectList("uni-community-mapping.selectMarkAnsInfo",param);	
				// 이미지 다건수만큼 loop 수행해서 json 만듦
