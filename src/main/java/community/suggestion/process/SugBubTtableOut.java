...
public class SugBubTtableOut {    
	
	// MAIN 생성용 OBJECT
	private JSONObject jObjRtn = new JSONObject();
	
	public SugBubTtableOut (JSONObject jobj) throws IOException {
	//public LonginTtableOut (String userId,String userType,String userPasswrd) throws IOException {
		...
		try {
			...
			{
				// 건의 게시판 생성일련번호 채번
				long creSeq = session.selectOne("uni-community-mapping.selectSugBubSeq");
				param.put("CRE_SEQ", creSeq);
				
				// 건의게시판 등록
				session.insert("uni-community-mapping.insertSugBubInfo",param);
			...
			if ("02".equals(jobj.getString("PROC_TYPE")))
			{
				...
				session.update("uni-community-mapping.updateSugBubInfo",param);
			}
			...
			if ("03".equals(jobj.getString("PROC_TYPE")))
			{
				...
				// 건의게시판 답변 삭제
				session.delete("uni-community-mapping.deleteSugAnsAllInfo",param);
			}
