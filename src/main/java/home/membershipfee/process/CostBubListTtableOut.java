...
public class CostBubListTtableOut {    
	
	private JSONObject jobj = new JSONObject();
	
	public CostBubListTtableOut (Map<String, Object> param) throws IOException {
	//public LonginTtableOut (String userId,String userType,String userPasswrd) throws IOException {
		...
		try {
			...
			rtnList = session.selectList("uni-home-mapping.selectCostBubInfo",param);	
			...
			for (int i=0; i < rtnList.size() ;i++)
			{
				...
				jObjSub.put("IN_COST_AMT", rtnList.get(i).get("IN_COST_AMT"));
				jObjSub.put("OUT_COST_AMT", rtnList.get(i).get("OUT_COST_AMT"));
