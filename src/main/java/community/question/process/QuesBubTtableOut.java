...
public class QuesBubTtableOut {    
	
	// MAIN 생성용 OBJECT
	private JSONObject jObjRtn = new JSONObject();
	
	public QuesBubTtableOut (JSONObject jobj) throws IOException {
	//public LonginTtableOut (String userId,String userType,String userPasswrd) throws IOException {
		...
		try {
			...
			{
				// 자유 게시판 생성일련번호 채번
				long creSeq = session.selectOne("uni-community-mapping.selectQuesBubSeq");
				param.put("CRE_SEQ", creSeq);
				
				// 자유게시판 등록
				session.insert("uni-community-mapping.insertQuesBubInfo",param);
			...
			if ("02".equals(jobj.getString("PROC_TYPE")))
			{
				...
				session.update("uni-community-mapping.updateQuesBubInfo",param);
			}
			...
			if ("03".equals(jobj.getString("PROC_TYPE")))
			{
				param.put("CRE_SEQ", jobj.get("CRE_SEQ"));
				// 자유게시판 삭제
				session.delete("uni-community-mapping.deleteQuesBubInfo",param);	
				// 자유게시판 답변 삭제
				session.delete("uni-community-mapping.deleteQuesAnsAllInfo",param);
			}
