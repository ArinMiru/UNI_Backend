...
public class SugBubListTtableOut {    
	
	private JSONObject jobj = new JSONObject();
	
	public SugBubListTtableOut (Map<String, Object> param) throws IOException {
	//public LonginTtableOut (String userId,String userType,String userPasswrd) throws IOException {
		...
		try {
			...
			rtnList = session.selectList("uni-community-mapping.selectSugBubInfo",param);	
			...
			for (int i=0; i < rtnList.size() ;i++)
			{
				...
				rtnListSub2 = session.selectList("uni-community-mapping.selectSugAnsInfo",param);	
