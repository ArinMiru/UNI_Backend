...
public class FreeAnsBubTtableOut {    
	
	// MAIN 생성용 OBJECT
	private JSONObject jObjRtn = new JSONObject();
	
	public FreeAnsBubTtableOut (JSONObject jobj) throws IOException {
	//public LonginTtableOut (String userId,String userType,String userPasswrd) throws IOException {
		...
		try {
			...
			{
				// 자유 게시판 답변 생성일련번호 채번
				long ansSeq = session.selectOne("uni-community-mapping.selectFreeAnsBubSeq");
				param.put("ANS_SEQ", ansSeq);
				
				// 자유게시판 답변 등록
				session.insert("uni-community-mapping.insertFreeAnsBubInfo",param);
			...
			if ("02".equals(jobj.getString("PROC_TYPE")))
			{
				...
				session.update("uni-community-mapping.updateFreeAnsBubInfo",param);
			}
			...
			if ("03".equals(jobj.getString("PROC_TYPE")))
			{
				// 자유게시판 답변 삭제
				param.put("ANS_SEQ", jobj.get("ANS_SEQ"));
				session.delete("uni-community-mapping.deleteFreeAnsInfo",param);
			}
